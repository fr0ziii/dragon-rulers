...
from src.api.models.swarm import Swarm, SwarmCreate
...
async def create_swarm(swarm_create: SwarmCreate):
    ...
    try:
        data, count = supabase.table("swarms").insert(swarm_create.dict()).execute()
        swarm_data = data[1][0]
...
async def list_swarms():
    ...
    try:
        data, count = supabase.table("swarms").select("*").execute()
        swarms_data = data[1]
    except Exception as e:
...
async def get_swarm(swarm_id: UUID):
    ...
    try:
        data, count = supabase.table("swarms").select("*").eq("id", str(swarm_id)).execute()
...
async def update_swarm(swarm_id: UUID, swarm_create: SwarmCreate):
    ...
    try:
        data, count = supabase.table("swarms").update(swarm_create.dict()).eq("id", str(swarm_id)).execute()
        swarm_data = data[1][0]
    except Exception as e:
...
async def delete_swarm(swarm_id: UUID):
    ...
    try:
        data, count = supabase.table("swarms").delete().eq("id", str(swarm_id)).execute()
    except Exception as e:
